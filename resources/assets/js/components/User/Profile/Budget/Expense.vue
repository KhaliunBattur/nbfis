<template>
    <div class="col-lg-6">
        <h4>Өрхийн сарын нийт зарлага</h4>
        <table v-bind:class="loading ? 'loading table table-bordered' : 'table table-bordered'">
            <tbody>
            <tr>
                <td><span class="text text-black">#</span></td>
                <td>
                    <input type="text" class="form-control input-sm" v-auto data-type="expense" v-model="expense.name" data-target="#expense-input" v-on:selected="setData(expense, '#expense-input')" placeholder="Зарлагын төрөл">
                    <input type="hidden" v-model="expense.name" id="expense-input" />
                </td>
                <td><input type="number" class="form-control input-sm" v-model="expense.expense" placeholder="Дүн"></td>
                <td>
            ...
            <tr v-for="(b, index) in expenses">
                ...
                <td>
                    <input type="text" class="form-control input-sm" v-model="b.name" v-auto data-type="expense" :data-target="'#expense-input' + b.id" :value="b.name" v-on:selected="setData(b, '#expense-input' + b.id)">
                    <input type="hidden" v-model="b.name" :id="'expense-input' + b.id" />
                </td>
                <td><input type="number" class="form-control input-sm" v-model="b.expense"></td>
                <td>
                    <div class="btn-group">
